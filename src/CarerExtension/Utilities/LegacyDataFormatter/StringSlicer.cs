...
/// <summary>
/// 文字列を任意の長さでスライスするための構造体です。
/// スライスしたデータ長を保持することで、繰り返しスライスした際に、文字列を部分的に切り取ったようなデータを返します。
/// </summary>
/// <param name="buffer">管理する文字列</param>
public ref struct StringSlicer(ReadOnlySpan<char> buffer)
{
    ...
    #region constructor
    /// <summary>
    /// 文字列を任意の長さでスライスするための構造体を、コピーするためのコンストラクタです。
    /// </summary>
    /// <param name="slicer">コピー元の<see cref="StringSlicer"/></param>
    ...
    #region method
    /// <summary>
    /// 現在のオブジェクトを新しい<see cref="StringSlicer"/>として複製します。
    /// </summary>
    /// <returns>複製した<see cref="StringSlicer"/></returns>
